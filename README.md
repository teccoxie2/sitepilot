...
- `CPA_BASE_URL` / `CPA_API_KEY` / `CPA_GATE_TOKEN`：本地 [CLIProxyAPI](https://github.com/router-for-me/CLIProxyAPI)（管理页如 `http://192.168.52.81:8317/management.html`）。核算台会改写成 OpenAI 兼容入口 `…/v1`，用客户端密钥调 `/v1/models` 与 `/v1/chat/completions`，并带上门禁头 `X-CPA-Gate`。管理页登录密码不要当成模型密钥或门禁令牌。图纸推导默认模型为 `gpt-5.6-luna`。Vercel 生产环境打不到局域网 `192.168.52.81`；在能访问该地址的机器上用 `cpa-tunnel/docker-compose.cloudflared.yml` 把 **gate**（只放行 `/v1`）暴露为 HTTPS 后，用 `scripts/link-public-cpa.sh` 写入项目环境变量。未配置时就绪接口保持 `configured: false`，不编造模型结果。
...
图纸 / Estimator 的大模型需要公网可达的 CPA `/v1`。Vercel 访问不到 `192.168.52.81:8317`。在能访问该地址的电脑上用 Cloudflare Tunnel 暴露（见 `cpa-tunnel/README.md`，不用 Fly）。公网只到本地 **gate**：路径白名单 `/v1`、请求头 `X-CPA-Gate`，可选 `allowlist.txt` 的 IP/CIDR。然后：
...
脚本会带上门禁请求公网 `/v1/models`；失败则退出，不会写入假地址。`CPA_API_KEY` 是 CPA **客户端密钥**，不是管理页登录密码。未配置时 `GET /engine/drawings/verify/ready` 为 `configured: false`。

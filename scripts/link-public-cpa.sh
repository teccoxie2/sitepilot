...
#
# Usage (on a machine that can reach the public CPA HTTPS):
#   CPA_BASE_URL=https://your-tunnel.example CPA_API_KEY='client-key' ./scripts/link-public-cpa.sh
...
if [[ "$http_code" == "401" || "$http_code" == "403" ]]; then
  echo "已连上 ${raw}，但密钥被拒绝（HTTP ${http_code}）。请换 CPA 客户端密钥，不要用管理页密码。" >&2
  exit 1
fi
